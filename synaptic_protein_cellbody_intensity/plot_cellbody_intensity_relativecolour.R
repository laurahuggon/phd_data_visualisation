...
parent_filepath_2 = "/Users/laurahuggon/Library/CloudStorage/OneDrive-King'sCollegeLondon/phd/lab/imaging/isim/imaging_data_y1/syp_stx/analysis_nis_elements/"
...
full_filename_2 = paste0(parent_filepath_2, marker, ".csv")
staining_to_imaging = read_csv(full_filename_2)
...
nis_elements_df = nis_elements_df %>%
  left_join(staining_to_imaging %>% select(Genotype, DIFF, DIV, Staining_to_Imaging),
            by = c("Genotype" = "Genotype", "DIFF" = "DIFF", "DIV" = "DIV"))
...
mean_by_sample = function(data, column_name) {
  ...
  result = data %>%
    group_by(Genotype, DIFF,Staining_to_Imaging) %>%
    summarise(
...
spread_means <- sample_means %>%
  select(Genotype, DIFF, Staining_to_Imaging) %>%
  spread(key = Genotype, value = Staining_to_Imaging)

# Calculate the difference between Q331K and WT
spread_means <- spread_means %>%
  mutate(Difference = `Q331K` - `WT`)

# Join the difference back to the original dataframe
sample_means <- sample_means %>%
  left_join(spread_means %>% select(DIFF, Difference), by = "DIFF")
...
plot_data = function(group_data, sample_data, annotation_data, x = "Genotype", sd = "SD") {
  ...
  # Overlay individual data points with different shapes for DIFF and conditional coloring for Q331K based on Difference
  p = p + geom_point(data = sample_data %>% filter(Genotype == "WT"), 
                     aes_string(x = x, 
                                y = "N_Mean", 
                                shape = "DIFF"), 
                     position = position_dodge(0), size = 1.5, stroke = 1.5, color = "black") + # WT points without color gradient
    scale_shape_manual(values = diff_shapes) +  # Adjust shape values if needed
    
    geom_point(data = sample_data %>% filter(Genotype == "Q331K"), 
               aes_string(x = x,
                          y = "N_Mean",
                          shape = "DIFF", 
                          color = "Difference"), 
               position = position_dodge(0), size = 1.5, stroke = 1.5) + # Q331K points with color gradient
    scale_shape_manual(values = diff_shapes) +  # Adjust shape values if needed
    
    # Add scale for the color gradient
    scale_color_gradient2(low = "blue", mid = "limegreen", high = "red", midpoint = 0)
...
png(paste0(parent_filepath, relative_filepath, marker, "_cellbody_", measurement, "_raw_relativecolour.png"), width=825, height=1335, res=300)
