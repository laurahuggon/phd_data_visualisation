...
parent_filepath = "/Users/k21224575/Library/CloudStorage/OneDrive-King'sCollegeLondon/phd/lab/imaging/isim/imaging_data_y1/syp_stx/analysis_nis_elements/cellbody_intensity/"
...
if (marker == "synaptophysin") {
  filename = "syp/PRE_cellbody_intensity_syp.csv"
} else if (marker == "Homer-1") {
  filename = "hmr/POST_cellbody_intensity_hmr.csv"
} else if (marker == "syntaxin-1A") {
  filename = "stx/PRE_cellbody_intensity_stx.csv"
} else if (marker == "PSD-95") {
  filename = "psd/POST_cellbody_intensity_psd.csv"
}
...
if(normalise == TRUE) {
  ...
  dir = sub("^([^/]+/).*", "\\1", filename)
  # Save plot
  ggsave(paste0(parent_filepath, dir, marker, "_cell-body_mean-intensity_norm_pooled.png"), plot=plot, width=2.05, height=3.5, dpi=300, bg="white")
  # Export test results
  # Define file path for saving CSVs
  csv_path = paste0(parent_filepath, dir, "stats/", marker, "_cell-body_mean-intensity_norm_pooled.csv")
  ...
  dir = sub("^([^/]+/).*", "\\1", filename)
  # Save plot
  ggsave(paste0(parent_filepath, dir, marker, "_cell-body_mean-intensity_raw_pooled.png"), plot=plot, width=2.1, height=3.5, dpi=300, bg="white")
  # Export test results
  # Define file path for saving CSVs
  csv_path = paste0(parent_filepath, dir, "stats/", marker, "_cell-body_mean-intensity_raw_pooled.csv")
  # Export the test results to CSV files
  write.csv(test_result, csv_path, row.names=FALSE)
}
