...
parent_filepath = "/Users/laurahuggon/Library/CloudStorage/OneDrive-King'sCollegeLondon/phd/lab/imaging/isim/imaging_data_y1/syp_stx/analysis_nis_elements/puncta_intensity/"
...
if (marker == "UNC13A") {
  ...
  filename = paste0("COLOC", entity, "_puncta_intensity_", substr(relative_filepath, 1, 3), ".csv")
} else if (marker == "synaptophysin" | marker == "PSD-95") {
  relative_filepath = "highdensity/"
  filename = paste0("COLOC", entity, "_puncta_intensity_", substr(relative_filepath, 1, 11), ".csv")
}
...
# Normalise SumIntensity values to TotalNeuriteVolume
# nis_elements_df = nis_elements_df %>%
#   mutate(
#     Mean_SumIntensity = PRE_SumIntensity / `TotalNeuriteBodyVolume(BTUB)`
#     )
...
# Define column name
column_name = paste0("Mean_", measurement, "ObjIntensity")
...
plot_data = function(group_data, sample_data, annotation_data, x = "Genotype", sd = "SD") {
  ...
  p = ggplot(group_data, aes_string(x = x,
    ...
    labs(title = "Puncta",
         x = "",
         y = paste0(measurement, "Intensity of ", marker, " (a.u.)"),
         fill = x) +
...
png(paste0(parent_filepath, relative_filepath, marker, "_puncta_", measurement, "intensity_raw.png"), width=825, height=1335, res=300)
