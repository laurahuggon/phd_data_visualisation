...
if(normalise == TRUE) {
  ...
  dir = sub("^([^/]+/).*", "\\1", filename)
  # Save plot
  ggsave(paste0(parent_filepath, dir, marker, "_puncta_mean-intensity_norm.png"), plot=plot, width=2.05, height=3.5, dpi=300, bg="white")
  # Export test results
  # Define file path for saving CSVs
  csv_path = paste0(parent_filepath, dir, "stats/", marker, "_pucnta_mean-intensity_norm.csv")
  ...
  dir = sub("^([^/]+/).*", "\\1", filename)
  # Save plot
  ggsave(paste0(parent_filepath, dir, marker, "_puncta_mean-intensity_raw.png"), plot=plot, width=2.1, height=3.5, dpi=300, bg="white")
  # Export test results
  # Define file path for saving CSVs
  csv_path = paste0(parent_filepath, dir, "stats/", marker, "_puncta_mean-intensity_raw.csv")
  # Export the test results to CSV files
  write.csv(test_result, csv_path, row.names=FALSE)
}