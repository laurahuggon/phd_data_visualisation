...
parent_filepath = "/Users/k21224575/Library/CloudStorage/OneDrive-King'sCollegeLondon/phd/lab/imaging/isim/imaging_data_y1/syp_stx/analysis_nis_elements/puncta_intensity/"
...
if (marker == "synaptophysin") {
  filename = "syp/COLOCPRE_puncta_intensity_syp.csv"
} else if (marker == "Homer-1") {
  filename = "hmr/COLOCPOST_puncta_intensity_hmr.csv"
} else if (marker == "syntaxin-1A") {
  filename = "stx/COLOCPRE_puncta_intensity_stx.csv"
} else if (marker == "PSD-95") {
  filename = "psd/COLOCPOST_puncta_intensity_psd.csv"
}
...
if(normalise == TRUE) {
  nis_elements_df = nis_elements_df %>%
    mutate(
      mean_intensity = Mean_MeanObjIntensity / BTUB_MeanIntensity
    )
} else {
  nis_elements_df = nis_elements_df %>%
    mutate(
      mean_intensity = Mean_MeanObjIntensity
    )
}
...
if(normalise == TRUE) {
  ...
  dir = sub("^([^/]+/).*", "\\1", filename)
  # Save plot
  ggsave(paste0(parent_filepath, dir, marker, "_puncta_mean-intensity_norm_pooled.png"), plot=plot, width=2.05, height=3.5, dpi=300, bg="white")
  # Export test results
  # Define file path for saving CSVs
  csv_path = paste0(parent_filepath, dir, "stats/", marker, "_pucnta_mean-intensity_norm_pooled.csv")
  ...
  dir = sub("^([^/]+/).*", "\\1", filename)
  # Save plot
  ggsave(paste0(parent_filepath, dir, marker, "_puncta_mean-intensity_raw_pooled.png"), plot=plot, width=2.1, height=3.5, dpi=300, bg="white")
  # Export test results
  # Define file path for saving CSVs
  csv_path = paste0(parent_filepath, dir, "stats/", marker, "_puncta_mean-intensity_raw_pooled.csv")
  # Export the test results to CSV files
  write.csv(test_result, csv_path, row.names=FALSE)
}
