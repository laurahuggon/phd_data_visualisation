...
parent_filepath = "/Users/laurahuggon/Library/CloudStorage/OneDrive-King'sCollegeLondon/phd/lab/imaging/isim/imaging_data_y1/syp_stx/analysis_nis_elements/"

relative_filepath_1 = "synapse_morphology/n_1-3/stx_coloc/"
filename_1 = "COLOCPRE_BTUB_1-24.csv"

relative_filepath_2 = "normalisation/"
filename_2 = "mean_intensity_normalisation_stx_hmr.csv"
...
full_filename_1 = paste0(parent_filepath, relative_filepath_1, filename_1)
full_filename_2 = paste0(parent_filepath, relative_filepath_2, filename_2)

nis_elements_df = read_csv(full_filename_1)
normalisation_df = read_csv(full_filename_2)
...
# Remove ZStackIndex column from normalisation_df
normalisation_df = normalisation_df %>%
  select(-ZStackIndex, -Filename)

# Join dataframes
nis_elements_df = cbind(nis_elements_df, normalisation_df)

# Normalise MeanVolumeColoc column to β-tubulin intensity
nis_elements_df = nis_elements_df %>%
  mutate(
    MeanVolumeColoc = MeanVolumeColoc / BTUB_MeanIntensity
  )
