...
relative_filepath = "24.05.03_vamp_snap/"
...
perform_test = function(data) {
  ...
  # Extract values grouped by Replicate using `Normalized Signal`
  # Test for normality using Shapiro-Wilk test
  normality_result = by(data$`Normalized Signal`, data$Replicate, shapiro.test)
  ...
  if (!wt_normal || !q331k_normal) { # If either variable is set to `FALSE`
    ...
    test_result = wilcox.test(`Normalized Signal` ~ Replicate, data = data) # ``Normalized Signal` ~ Replicate` specifies that you want to compare values in the ``Normalized Signal`` column grouped by `Replicate`
  } else {
    # Test for equal variances using F-test
    variance_test = var.test(`Normalized Signal` ~ Replicate, data = data)
    ...
    if (!var_equal) { # If `var_equal` variable is set to `FALSE`
      ...
      test_result = t.test(`Normalized Signal` ~ Replicate, data = data, var.equal = FALSE)
    } else {
      # If variances are equal, perform t-test with var.equal = TRUE
      message = "Data has equal variance; perform Student's t-test"
      test_result = t.test(`Normalized Signal` ~ Replicate, data = data, var.equal = TRUE)
    }
  }
...
# Perform t-test
result = perform_test(empiria_data)
...
annotation = annotation %>%
  mutate(
    max_y = max(group_means$Group_Mean + group_means$SD)
  )
...
plot_normalised = function(group_data, sample_data, annotation_data, x = "Replicate", sd = "SD") {
  ...
  if (!all(c(x, y = "Group_Mean", sd) %in% names(group_data))) {
  ...
  p = ggplot(group_data, aes_string(x = x,
    ...
    # Error bars
    geom_errorbar(aes(ymin = group_data[["Group_Mean"]] - group_data[[sd]],
                      ymax = group_data[["Group_Mean"]] + group_data[[sd]]),
                  width = 0.2,
    ...
    # Plot appearance
    my_theme() +
    scale_y_continuous(limits = c(0, 12000), expand = c(0, 0))  # Setting both multiplier and add-on to 0
...
# Make plot
plot = plot_normalised(group_means, empiria_data, annotation)
...
png(paste0("/Users/laurahuggon/Library/CloudStorage/OneDrive-King'sCollegeLondon/phd/lab/wb/i3neuron_synapse/normalised_signal/", protein_name, "_wb_plot_colour_signal.png"), width=825, height=1335, res=300)
