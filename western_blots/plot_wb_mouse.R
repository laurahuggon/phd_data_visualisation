...
control_mean = group_means %>%
  filter(Replicate == "NTg") %>% # Where `Replicate` equals `NTg`
...
plot_normalised = function(group_data, sample_data, annotation_data, x = "Replicate", sd = "SD") {
  ...
  if (nrow(annotation_data) > 0) {
    ...
    # `x = 1.5` is used to position the text centrally between the two bars -> assumes that Replicate has 2 ordered levels which correspond to position 1 and 2
