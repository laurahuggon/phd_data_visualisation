...
# Split Replicate column
empiria_data = empiria_data %>%
  separate(Replicate, into = c("Genotype", "DIFF"), sep = " ") %>%
  mutate(Genotype = str_replace_all(Genotype, "QK", "Q331K"))

# Define Genotype as a factor with levels
empiria_data$Genotype = factor(empiria_data$Genotype, levels = c("WT", "Q331K"))

# Reshape data into long format
long_data = empiria_data %>%
  pivot_longer(-c(Genotype, DIFF), names_to = "Protein", values_to = "Signal") %>%
  arrange(Genotype) %>%
  arrange(Protein)
...
plot_data = function(group_data, group_col_name, individual_data, individual_col_name, x, facet_grouping, test_results = test_result) {
  ...
  diff_shapes = c("9" = 21, "10" = 22, "11" = 24)
